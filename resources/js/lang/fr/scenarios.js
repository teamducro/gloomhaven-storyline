export default {
    "gh-1": {
        name: "Le Tertre Noir",
    },
    "gh-2": {
        name: "Le Repaire sous le Tertre",
        rewards: ["10 gold each", "+1 prosperity"],
    },
    "gh-3": {
        name: "Le Campement Inox",
        rewards: ["15 gold each", "+1 prosperity"],
    },
    "gh-4": {
        name: "La Crypte des Damnés",
    },
    "gh-5": {
        name: "La Crypte dans les Ruines",
    },
    "gh-6": {
        name: "La Crypte en Déclin",
        rewards: ["5 gold each"],
    },
    "gh-7": {
        name: "La Grotte foisonnante",
    },
    "gh-8": {
        name: "L'Entrepôt de Havrenuit",
        rewards: ["+2 reputation"],
    },
    "gh-9": {
        name: "La Mine de Diamant",
        rewards: ["20 gold each", "+1 prosperity"],
    },
    "gh-10": {
        name: "La Dimension du Pouvoir Elementaire",
    },
    "gh-11": {
        name: "La Place de Havrenuit A",
        rewards: [
            "15 gold each",
            "-2 reputation",
            "+2 prosperity",
            "“Skullbane Axe” design (Item 113)",
        ],
    },
    "gh-12": {
        name: "La Place de Havrenuit B",
        rewards: ["+4 reputation", "“Skullbane Axe” design (Item 113)"],
    },
    "gh-13": {
        name: "Le Temple du Prophète",
    },
    "gh-14": {
        name: "La Caverne Gelée",
    },
    "gh-15": {
        name: "Le Sanctuaire de Puissance",
        rewards: ["20 experience each"],
    },
    "gh-16": {
        name: "Le Col de la Montagne",
    },
    "gh-17": {
        name: "L'Ile Perdue",
        rewards: ["25 gold each"],
    },
    "gh-18": {
        name: "Les Egouts Abandonnés",
    },
    "gh-19": {
        name: "La Crypte Oubliée",
        rewards: ["+1 prosperity"],
    },
    "gh-20": {
        name: "Le Sanctuaire de la Nécromancienne",
        rewards: ["20 gold each", "+2 reputation", "+1 prosperity"],
    },
    "gh-21": {
        name: "L'Autel Infernal",
        rewards: [
            "50 gold each",
            "+1 prosperity",
            "Add City Event 78 to the deck",
        ],
    },
    "gh-22": {
        name: "Le Temple des Eléments",
    },
    "gh-23": {
        name: "Les Ruines des Profondeurs",
    },
    "gh-24": {
        name: "La Chambre des Echos",
    },
    "gh-25": {
        name: "L'Ascension du Rideau de Glace",
    },
    "gh-26": {
        name: "L'Ancienne Citerne",
        rewards: ["10 gold each", "+1 reputation", "+2 prosperity"],
    },
    "gh-27": {
        name: "La faille dans les Ruines",
        rewards: [
            "+1 prosperity",
            "100 gold each This money must immediately be spent on enhancements",
        ],
    },
    "gh-28": {
        name: "La Chambre du Rituel de l'Outre-Monde",
    },
    "gh-29": {
        name: "Le Sanctuaire de l'Ombre",
        rewards: ["15 experience each"],
    },
    "gh-30": {
        name: "Le Mausolée des Profondeurs",
        rewards: ["10 gold each"],
    },
    "gh-31": {
        name: "La Dimension de la Nuit",
    },
    "gh-32": {
        name: "Le Bois Abandonné",
    },
    "gh-33": {
        name: "L'Armurerie Savvas",
    },
    "gh-34": {
        name: "Le Sommet Calciné",
        rewards: ["20 gold each", "+2 reputation", "+1 prosperity"],
    },
    "gh-35": {
        name: "Les Remparts de Havrenuit A",
        rewards: [
            "30 gold each",
            "5 reputation",
            "-2 prosperity",
            "Add City Event 79 to the deck",
        ],
    },
    "gh-36": {
        name: "Les Remparts de Havrenuit B",
        rewards: [
            "10 gold each",
            "+4 reputation",
            "Add City Event 78 to the deck",
        ],
    },
    "gh-37": {
        name: "La Tranchée de l'Ombre",
    },
    "gh-38": {
        name: "L'Enclos des Esclaves",
        rewards: ["+1 reputation"],
    },
    "gh-39": {
        name: "Le Précipice Périlleux",
        rewards: ["10 experience each"],
    },
    "gh-40": {
        name: "L'Ancien Réseau de Défense",
    },
    "gh-41": {
        name: "Le Tombeau Erodé",
        rewards: [
            "50 gold each",
            "25 experience each",
            "2 ✔ each",
            "+2 prosperity",
        ],
    },
    "gh-42": {
        name: "Le Royaume de la Voix",
        rewards: ["{NOTE_42}"],
    },
    "gh-43": {
        name: "Le Nid des Wyvernes",
    },
    "gh-44": {
        name: "Assaut Tribal",
        rewards: ["Open envelope {DS}", "+2 reputation"],
    },
    "gh-45": {
        name: "Le Marais des Rebelles",
        rewards: ["20 gold each", "-2 reputation"],
    },
    "gh-46": {
        name: "Le Pic des Cauchemars",
    },
    "gh-47": {
        name: "L'Antre de l'Oeil Aveugle",
    },
    "gh-48": {
        name: "La Forêt Lugubre",
    },
    "gh-49": {
        name: "Le Campement des Rebelles",
        rewards: ["50 gold each", "-3 reputation"],
    },
    "gh-50": {
        name: "La Forteresse Fantôme",
        rewards: ["+3 reputation", "-2 prosperity"],
    },
    "gh-51": {
        name: "Le Néant",
        rewards: [
            "+5 reputation",
            "+5 prosperity",
            "Add City Event 81 to the deck",
            "Add Road Event 69 to the deck",
        ],
    },
    "gh-52": {
        name: "La Cave Toxique",
    },
    "gh-53": {
        name: "Le Sous-sol de la Crypte",
        rewards: [
            "The Seeker of Xorn receives: “Staff of Xorn” (Item 114) The Seeker cannot sell this item.",
        ],
    },
    "gh-54": {
        name: "Le Palais de Glace",
        rewards: [
            "Immediately retire the Seeker of Xorn",
            "Add City and Road Events 59 to the decks instead of the normal retirement events for the class",
            "Open envelope {PH}",
        ],
    },
    "gh-55": {
        name: "Le fourré Brumeux",
        rewards: ["10 collective gold"],
    },
    "gh-56": {
        name: "Le Bois du Bandit",
        rewards: [
            "“Take Back the Trees” quest complete",
            "Open envelope {DS}",
            "10 gold each",
            "+2 reputation",
        ],
    },
    "gh-57": {
        name: "Enquête",
        rewards: ["+1 reputation"],
    },
    "gh-58": {
        name: "La Planque Ensanglantée",
        rewards: [
            "“Vengeance” quest complete",
            "Open envelope X",
            "+2 reputation",
        ],
    },
    "gh-59": {
        name: "Le Bosquet Oublié",
    },
    "gh-60": {
        name: "Le laboratoire d'Alchimie",
        rewards: [
            "“Finding the Cure” quest complete",
            "Open envelope X",
            "+1 prosperity",
        ],
    },
    "gh-61": {
        name: "Le Phare Evanescent",
    },
    "gh-62": {
        name: "La fosse des Ames",
        rewards: [
            "“The Fall of Man” quest complete",
            "Open envelope {NS}",
            "10 experience each",
        ],
    },
    "gh-63": {
        name: "Le Cratère de Magma",
        rewards: ["15 gold each"],
    },
    "gh-64": {
        name: "Le Lagon Sous-marin",
        rewards: ["10 experience each"],
    },
    "gh-65": {
        name: "La Mine de Soufre",
        rewards: ["“Ancient Drill” design (Item 112)"],
    },
    "gh-66": {
        name: "La Crique Mécanique",
    },
    "gh-67": {
        name: "La Bibliothèque Arcanique",
        rewards: ["“Power Core” (Item 132)"],
    },
    "gh-68": {
        name: "La Lande Toxique",
        rewards: ["2 “Major Healing Potions” (Item 027)"],
    },
    "gh-69": {
        name: "Le Puits de l'Infortuné",
        rewards: ["15 gold each"],
    },
    "gh-70": {
        name: "L'Ile Enchaînée",
        rewards: ["+2 prosperity"],
    },
    "gh-71": {
        name: "Les Plateaux Venteux",
        rewards: ["2 “Major Power Potions” (Item 041)"],
    },
    "gh-72": {
        name: "Le Bosquet Suintant",
        rewards: ["+1 reputation", "+1 prosperity"],
    },
    "gh-73": {
        name: "La Crête de l'Eboulement",
        rewards: ["+1 reputation"],
    },
    "gh-74": {
        name: "Le Navire Marchand",
        rewards: ["10 gold each", "+2 prosperity"],
    },
    "gh-75": {
        name: "Le Cimetière en friche",
        rewards: ["60 gold each"],
    },
    "gh-76": {
        name: "La Ruche des Persécuteurs",
    },
    "gh-77": {
        name: "Le Caveau des Secrets",
        rewards: ["5 experience each"],
    },
    "gh-78": {
        name: "Le fossé Sacrificiel",
        rewards: ["+3 reputation"],
    },
    "gh-79": {
        name: "Le Temple Perdu",
        rewards: ["15 gold each"],
    },
    "gh-80": {
        name: "La Forteresse de la Vigie",
        rewards: ["10 experience each"],
    },
    "gh-81": {
        name: "Le Temple de lEclipse",
        rewards: ["10 experience each"],
    },
    "gh-82": {
        name: "La Montagne Ardente",
        rewards: [["+1 reputation"], ["-1 reputation", "-2 prosperity"]],
    },
    "gh-83": {
        name: "Les Ombres Intérieures",
        rewards: ["40 collective gold"],
    },
    "gh-84": {
        name: "La Grotte Cristalline",
        rewards: ["“Resonant Crystal” (Item 133)", "+1 prosperity"],
    },
    "gh-85": {
        name: "Le Temple Solaire",
        rewards: ["“Orb of Dawn” (Item 121)"],
    },
    "gh-86": {
        name: "Le Village Tourmenté",
        rewards: ["+2 reputation"],
    },
    "gh-87": {
        name: "La Crique Corrompue",
        rewards: ["+1 reputation", "+1 prosperity"],
    },
    "gh-88": {
        name: "La Dimension de l'Eau",
        rewards: ["“Staff of Summoning” (Item 120)"],
    },
    "gh-89": {
        name: "La Planque du Syndicat",
        rewards: ["50 collective gold"],
    },
    "gh-90": {
        name: "La faille Démoniaque",
        rewards: ["“Black Censer” (Item 128)"],
    },
    "gh-91": {
        name: "La Mêlée Sauvage",
        rewards: ["2 ✔ each"],
    },
    "gh-92": {
        name: "Bagarre dans l'Arrière-cour",
        rewards: ["10 gold each", "-3 reputation"],
    },
    "gh-93": {
        name: "Le Vaisseau Englouti",
        rewards: ["10 experience each"],
    },
    "gh-94": {
        name: "Le Nid des Vermlings",
    },
    "gh-95": {
        name: "L'Heure du Jugement",
        rewards: ["“Skull of Hatred” (Item 119)"],
    },
    "gh-202": {
        name: "Return to the Black Barrow",
        rewards: ["“Imposing Blade” (Item 134)"],
    },
    "gh-203": {
        name: "An Unfortunate Intrusion",
        rewards: ["“Focusing Ray” (Item 135)"],
    },
    "gh-204": {
        name: "Corrupted Laboratory",
        rewards: ["“Volatile Elixir” (Item 136)"],
    },
    "gh-205": {
        name: "Armory Heist",
        rewards: ["“Silent Stiletto” (Item 137)"],
    },
    "gh-206": {
        name: "Stone Defense",
        rewards: ["“Stone Charm” (Item 138)"],
    },
    "gh-207": {
        name: "Rodent Liberation",
        rewards: ["“Psychic Knife” (Item 139)"],
    },
    "gh-208": {
        name: "Caravan Escort",
        rewards: ["“Sun Shield” (Item 140)"],
    },
    "gh-209": {
        name: "Unnatural Insults",
        rewards: ["“Mask of Death” (Item 145)"],
    },
    "gh-210": {
        name: "Storage Fees",
        rewards: ["“Utility Belt” (Item 141)"],
    },
    "gh-211": {
        name: "Plane of Wild Beasts",
        rewards: ["“Phasing Idol” (Item 142)"],
    },
    "gh-212": {
        name: "Harvesting the Night",
        rewards: ["“Smoke Elixir” (Item 143)"],
    },
    "gh-213": {
        name: "Plagued Crypt",
        rewards: ["“Pendant of the Plague” (Item 144)"],
    },
    "gh-214": {
        name: "Battle of the Bards",
        rewards: ["“Master’s Lute” (Item 146)"],
    },
    "gh-215": {
        name: "Corrupted Hunt",
        rewards: ["“Cloak of the Hunter” (Item 147)"],
    },
    "gh-216": {
        name: "Aftermath",
        rewards: ["“Doctor’s Coat” (Item 148)"],
    },
    "gh-217": {
        name: "Elemental Secrets",
        rewards: ["“Elemental Boots” (Item 149)"],
    },
    "gh-218": {
        name: "The Caged Bear",
        rewards: ["“Staff of Command” (Item 150)"],
    },
    "gh-219": {
        name: "The Sands of Time",
        rewards: ["“Sword of the Sands” (Item 151)"],
    },
    "mp-103": {
        "name": "A Harrowing Deal",
        "rewards": [
            "“Ribbon of Friendship”"
        ]
    },
    "mp-104": {
        "name": "Horns in the Night",
        "rewards": [
            "“Crest of Frosthaven”"
        ]
    },
    "mp-105": {
        "name": "Escape from Corruption",
        "rewards": [
            "“Basin of Prophecy (A)”",
            "“Basin of Prophecy (B)”"
        ]
    },
    "mp-106": {
        "name": "Major Annoyance",
        "rewards": [
            "“No Nonsense Boots”"
        ]
    },
    "fc-96": {
        name: "Unexpected Visitors",
        rewards: [
            "10 experience each",
            "Add City and Road Events 82 to their respective decks",
        ],
    },
    "fc-97": {
        name: "Lore Untold",
        rewards: ["10 gold each", "Add City Events 88, 89 and 90 to the deck"],
    },
    "fc-98": {
        name: "Past in Flames",
        rewards: [
            [
                "10 gold each",
                "+1 reputation",
                "+1 prosperity",
                "Add City Event 86 to the deck",
            ],
            ["5 gold each", "+1 reputation"],
            ["5 gold each", "+1 reputation"],
        ],
    },
    "fc-99": {
        name: "Aftershocks",
        rewards: [
            ["5 gold each", "“Minor Antidote” (Item 153)"],
            ["10 gold each", "+1 prosperity", "“Minor Antidote” (Item 153)"],
        ],
    },
    "fc-100": {
        name: "Shifting Gears",
        rewards: [
            "Choose to gain one of the following sets: “Telescopic Lens” (Item 059) and “Ancient Drill” (Item 112), OR “Curious Gear” (Item 125) and “Remote Spider” (Item 126)",
        ],
    },
    "fc-101": {
        name: "Shrouded Crypt",
        rewards: ["20 collective gold", "+1 reputation", "+1 prosperity"],
    },
    "fc-102": {
        name: "Bazaar of Knowledge",
        rewards: [
            "15 experience each",
            "If no more than 2+C inhabitants were intimidated: +2 reputation Otherwise: -2 reputation",
        ],
    },
    "fc-103": {
        name: "Where It Is Needed",
        rewards: ["+2 prosperity", "Add City Event 87 to the deck"],
    },
    "fc-104": {
        name: "A Gaping Wound",
        rewards: ["“Major Mana Potion” (Item 048)"],
    },
    "fc-105": {
        name: "Monstrosities of a Cult",
        rewards: ["“Major Antidote” (Item 154)"],
    },
    "fc-106": {
        name: "Intricate Work",
        rewards: [
            [
                "175 gold each, This money must immediately be spent on any {WIND}, {ICE}, {FIRE}, {EARTH}, {LIGHT}, or {DARK} enhancements",
            ],
            [
                "175 gold each, This money must immediately be spent on any {WIND}, {ICE}, {FIRE}, {EARTH}, {LIGHT}, or {DARK} enhancements",
                "+ 1 prosperity",
            ],
        ],
    },
    "fc-107": {
        name: "Mechanical Genius",
        rewards: ["5 experience each"],
    },
    "fc-108": {
        name: "Prologue to the End",
        rewards: [
            ["“Curseward Armor” (Item 155)"],
            ["2 “Throwing Axes” (Item 161)"],
            ["“Curseward Armor” (Item 155)", "2 “Throwing Axes” (Item 161)"],
        ],
    },
    "fc-109": {
        name: "Epilogue of a War",
        rewards: ["10 experience each", "“Protective Charm” (Item 052)"],
    },
    "fc-110": {
        name: "A Circular Solution",
        rewards: ["“Crystal Tiara” (Item 163)"],
    },
    "fc-111": {
        name: "The Shackles Loosen",
        rewards: [
            [
                "5 experience each",
                "Add Rift Event 16 to the deck. Instead of shuffling this event into the whole deck, shuffle it into the top 3 cards",
            ],
            ["12 experience each", "1 ✓ each"],
        ],
    },
    "fc-112": {
        name: "The Bottom of It",
        rewards: [
            ["10 gold each", "5 experience each"],
            ["25 collective gold", "5 experience each", "1 ✓ each"],
        ],
    },
    "fc-113": {
        name: "The Lost Thread",
        rewards: [
            "135 gold each",
            "This money must immediately be spent on any Move {MOVE}, Range {RANGE} or Jump {JUMP} enhancements",
            "+1 prosperity",
        ],
    },
    "fc-114": {
        name: "Ink Not Yet Dry",
        rewards: [
            "Party Boon: Additional costs of enhancing an ability card based on the level of the card are now reduced by 5 gold per level.",
        ],
    },
    "fc-115": {
        name: "Future Uncertain",
        rewards: [
            "+3 reputation",
            "+1 prosperity",
            "Retire the Aesther Diviner {DR}",
            "Add City and Road Events 83 to their respective decks",
        ],
    },
    "fc-116": {
        name: "Forecast of the Inevitable",
        rewards: ["“Basin of Prophecy” (Item 164)"],
    },
    "fc-117": {
        name: "Tower to the Stars",
        rewards: [
            "15 gold each",
            "1 ✓ each",
            "All characters may add one new card of their class to their active card pool. The card selected must have a level equal to or less than the character's level minus 2.",
        ],
    },
    "jotl-1": {
        name: "Roadside Ambush",
    },
    "jotl-2": {
        name: "A Hole in the Wall",
        rewards: ["25 gold each", "Add items 1-13 to the shop"],
    },
    "jotl-3": {
        name: "The Black Ship",
        rewards: ["1 ✔ each"],
    },
    "jotl-4": {
        name: "A Ritual in Stone",
        rewards: ["“Mana Potion” (Item 14)"],
    },
    "jotl-5": {
        name: "A Deeper Understanding",
        rewards: ["25 experience each"],
    },
    "jotl-6": {
        name: "Corrupted Research",
        rewards: ["1 ✔ each"],
    },
    "jotl-7": {
        name: "Sunken Tumor",
        rewards: ["20 experience each"],
    },
    "jotl-8": {
        name: "Hidden Tumor",
    },
    "jotl-9": {
        name: "Explosive Evolution",
        rewards: ["20 experience each", "Add items 15-20 to the shop"],
    },
    "jotl-10": {
        name: "The Gauntlet",
        rewards: ["15 experience each"],
    },
    "jotl-11": {
        name: "Defiled Sewers",
        rewards: ["“Flea-Bitten Shawl” (Item 28)"],
    },
    "jotl-12": {
        name: "Beguiling Sewers",
        rewards: ["“Flea-Bitten Shawl” (Item 28)"],
    },
    "jotl-13": {
        name: "Vile Harvest",
        rewards: ["1 ✔ each"],
    },
    "jotl-14": {
        name: "Toxic Harvest",
        rewards: ["1 ✔ each"],
    },
    "jotl-15": {
        name: "Tainted Blood",
        rewards: ["10 gold each", "Add items 21-26 to the shop"],
    },
    "jotl-16": {
        name: "Mixed Results",
        rewards: ["15 experience each", "“Tower Shield” (Item 29)"],
    },
    "jotl-17": {
        name: "Red Twilight",
        rewards: ["25 experience each", "30 gold each"],
    },
    "jotl-18": {
        name: "The Heist",
        rewards: ["10 experience each"],
    },
    "jotl-19": {
        name: "Den of Thieves",
        rewards: ["“Ring of Strength” (Item 31)"],
    },
    "jotl-20": {
        name: "Misplaced Goods",
        rewards: ["10 gold each"],
    },
    "jotl-21": {
        name: "Agents of Chaos",
        rewards: ["15 gold each"],
    },
    "jotl-22": {
        name: "Unfriendly Message",
        rewards: ["“Steel Ring” (Item 33)"],
    },
    "jotl-23": {
        name: "Best of the Best",
        rewards: ["“Barbed Axe” (Item 34)"],
    },
    "jotl-24": {
        name: "Warding the Void",
        rewards: ["“Robes of Command” (Item 35)"],
    },
    "jotl-25": {
        name: "The Greatest Job in the World",
        rewards: ["“Jet Boots” (Item 36)"],
    },
    "fh-0": {
        name: "Howling in the Snow",
    },
    "fh-1": {
        name: "A Town in Flames",
        rewards: [
            ["Gain 2 morale"],
            ["Gain 3 morale"],
            ["Gain 4 morale"],
            ["Gain 5 morale"],
            ["Gain 6 morale"],
            ["Gain 7 morale"],
        ],
    },
    "fh-2": {
        name: "Algox Scouting",
        rewards: ["Gain 1 ✔ each"],
    },
    "fh-3": {
        name: "Algox Offensive",
        rewards: ["Gain 1 ✔ each"],
    },
    "fh-4": {
        name: "Heart of Ice",
        rewards: [
            [
                "Unlock {FF} class box",
                "Gain 1 morale",
                "Gain 1 prosperity",
                "Add 156.1 to the calendar in four weeks",
            ],
            [
                "Unlock {SD} class box",
                "Gain 1 morale",
                "Gain 1 prosperity",
                "Add 156.1 to the calendar in four weeks",
            ],
        ],
    },
    "fh-5": {
        name: "Frozen Crypt",
        rewards: ["Gain 2 collective {LUMBER}"],
    },
    "fh-6": {
        name: "Avalanche",
        rewards: ["Gain 2 collective {METAL}"],
    },
    "fh-7": {
        name: "Edge of the World",
        rewards: ["10 experience each"],
    },
    "fh-8": {
        name: "Crystal Trench",
        rewards: ["Gain 1 collective {LUMBER}", "Gain 2 collective {HIDE}"],
    },
    "fh-9": {
        name: "Glowing Catacombs",
        rewards: ["Gain 2 collective {CORPSECAP}"],
    },
    "fh-10": {
        name: "Crystal Enclosure",
        rewards: ["Gain 1 morale", "Gain 1 inspiration"],
    },
    "fh-11": {
        name: "Snowscorn Peak",
        rewards: ["Gain 1 morale", "Gain 1 inspiration"],
    },
    "fh-12": {
        name: "Temple Entrance",
        rewards: ["Gain 15 experience each"],
    },
    "fh-13": {
        name: "Frozen Fjord",
        rewards: ["Gain “Dead Raven” (Item 233)"],
    },
    "fh-14": {
        name: "Jagged Shoals",
        rewards: [
            "Choose one: Unlock {CR} class box OR unlock {DT} class box",
            "Add event WO-81 to the winter outpost deck",
        ],
    },
    "fh-15": {
        name: "Ancient Spire",
        rewards: [
            "Gain 1 collective {LUMBER}",
            "Gain 1 collective {METAL}",
            "Gain 1 collective {HIDE}",
        ],
    },
    "fh-16": {
        name: "Derelict Elevator",
        rewards: ["Gain 5 experience each", "Gain 1 inspiration"],
    },
    "fh-17": {
        name: "Haunted Vault",
        rewards: ["Gain 1 ✔ each"],
    },
    "fh-18": {
        name: "Crystal Fields",
        rewards: [
            "Gain 1 morale",
            "Gain one random item blueprint",
            "Add 122.3 to the calendar in four weeks",
        ],
    },
    "fh-19": {
        name: "Skyhall",
        rewards: [
            "Gain 1 morale",
            "Gain one random item blueprint",
            "Add 54.3 to the calendar in four weeks",
        ],
    },
    "fh-20": {
        name: "Temple of Liberation",
        rewards: ["Gain 15 gold each"],
    },
    "fh-21": {
        name: "Realm of Endless Frost",
        rewards: ["Gain 2 prosperity"],
    },
    "fh-22": {
        name: "Ice Floes",
        rewards: [
            "Lose 1 collective {LUMBER} for each set of five hexes with water tiles on tile 7-G",
            "Gain 15 experience each",
            "Add 62.2 to the calendar in two weeks",
        ],
    },
    "fh-23": {
        name: "Spire Basement",
        rewards: ["Gain one random item blueprint"],
    },
    "fh-24": {
        name: "Upper Spire",
        rewards: ["Gain one random item blueprint"],
    },
    "fh-25": {
        name: "Rusted Tunnels",
        rewards: [
            "Gain 2 collective {LUMBER}",
            "Gain 2 collective {METAL}",
            "Add 128.2 to the calendar in four weeks",
        ],
    },
    "fh-26": {
        name: "Quatryl Library",
        rewards: ["Gain 10 experience each"],
    },
    "fh-27": {
        name: "Depths of Delirium",
        rewards: [
            "Gain 10 experience each",
            "Gain 1 ✔ each",
            "Ancient Technology",
        ],
    },
    "fh-28": {
        name: "Summit Meeting",
        rewards: ["Lose 1 morale"],
    },
    "fh-29": {
        name: "War of the Spire A",
        rewards: [
            "Gain 3 morale",
            "Remove all {ALGOX} events from all outpost event decks",
            "Add event WO-68 to the winter outpost deck",
            "Add 46.3 to the calendar in five weeks",
        ],
    },
    "fh-30": {
        name: "War of the Spire B",
        rewards: [
            "Gain 3 morale",
            "Gain “Destroyer of the Snowspeakers” sticker",
            "Remove all {ALGOX} events from all outpost event decks",
            "Add event WO-69 to the winter outpost deck",
            "Add 34.2 to the calendar in five weeks",
        ],
    },
    "fh-31": {
        name: "Crackling Tunnel",
        rewards: ["Gain 5 experience each", "Gain 10 gold each"],
    },
    "fh-32": {
        name: "Ravens' Roost",
        rewards: ["Gain “Feathered Cloak” (Item 69) blueprint"],
    },
    "fh-33": {
        name: "Thawed Wood",
        rewards: [
            "Gain 5 experience each for each unrevealed dowsing rune",
            "Gain “Ember Energy Source” (Item 244)",
            "Add 135.1 to the calendar in three weeks",
        ],
    },
    "fh-34": {
        name: "Top of the Spire",
    },
    "fh-35": {
        name: "Scrap Pit",
        rewards: [
            "Gain 2 collective {LUMBER}",
            "Gain 2 collective {METAL}",
            "Gain 2 collective {HIDE}",
        ],
    },
    "fh-36": {
        name: "Buried Ducts",
        rewards: ["Gain 3 collective {LUMBER}", "Gain 3 collective {METAL}"],
    },
    "fh-37": {
        name: "The Dead Mile",
        rewards: ["Gain 3 collective {LUMBER}", "Gain 3 collective {METAL}"],
    },
    "fh-38": {
        name: "The Way Forward",
        rewards: [
            "Gain 10 experience each",
            "Gain 1 ✔ each",
            "Gain 1 inspiration",
        ],
    },
    "fh-39": {
        name: "Corrupted Camp",
        rewards: ["Lose 1 inspiration", "Gain 2 additional loot cards each"],
    },
    "fh-40": {
        name: "Relief Effort",
        rewards: ["Lose 1 inspiration", "Gain 2 additional loot cards each"],
    },
    "fh-41": {
        name: "Unfettered Shard",
        rewards: [
            "Gain 15 gold each",
            "Gain 2 collective {AXENUT}",
            "Ancient Technology",
        ],
    },
    "fh-42": {
        name: "Sunless Trench",
        rewards: [
            "Gain 2 ✔ each",
            "Add event B-16 to the boat deck",
            "Add 114.2 to the calendar in three weeks",
        ],
    },
    "fh-43": {
        name: "Overrun Barricade",
        rewards: ["Gain 10 experience each"],
    },
    "fh-44": {
        name: "Nerve Center",
        rewards: [
            "Gain 5 gold each",
            "Gain “Energizing Baton” (Item 73) blueprint",
        ],
    },
    "fh-45": {
        name: "Living Glacier",
        rewards: ["Gain 1 morale", "Gain 1 ✔ each"],
    },
    "fh-46": {
        name: "Dead Pass",
        rewards: ["Gain 1 morale", "Gain 10 experience each"],
    },
    "fh-47": {
        name: "Carrion Ridge",
        rewards: ["Gain 10 gold each", "Gain 2 collective {LUMBER}"],
    },
    "fh-48": {
        name: "Blizzard island",
        rewards: ["Gain 10 gold each", "Gain 2 collective {METAL}"],
    },
    "fh-49": {
        name: "Beneath Sea and Stone",
        rewards: ["Gain 5 experience each", "Gain 1 inspiration"],
    },
    "fh-50": {
        name: "Explosive Descent",
        rewards: [
            "Lose 2 inspiration",
            "Gain 4 collective {LUMBER}",
            "Gain 4 collective {METAL}",
        ],
    },
    "fh-51": {
        name: "Orphan's Halls",
        rewards: [
            "Gain 10 experience each",
            "Gain “Shock Grenades” (Item 74) blueprint",
        ],
    },
    "fh-52": {
        name: "Fleeting Permanence",
        rewards: ["Gain 1 morale", "Gain 1 inspiration"],
    },
    "fh-53": {
        name: "Underwater Throne",
        rewards: ["Gain 2 ✔ each"],
    },
    "fh-54": {
        name: "Among the Wreckage",
        rewards: ["Gain 2 ✔ each"],
    },
    "fh-55": {
        name: "Change of Heart",
        rewards: [
            "Gain 2 morale",
            "Gain 2 prosperity",
            "Unlock whichever class box is still locked: {FF} or {SD}",
            "Remove all {ALGOX} events from all outpost event decks",
            "Add events WO-66 and WO-67 to the winter outpost deck",
            "Add one +50 card to town guard deck",
        ],
    },
    "fh-56": {
        name: "Call of the Harbinger",
        rewards: [
            "Gain 1 morale",
            "Gain 1 inspiration",
            "Unlock {FF} class box",
            "Add event SO-51 to the summer outpost deck",
            "Add one +50 card to town guard deck",
        ],
    },
    "fh-57": {
        name: "Sanctuary of Snow",
        rewards: [
            "Gain 1 morale",
            "Gain 1 inspiration",
            "Unlock {SD} class box",
            "Add event WO-68 to the winter outpost deck",
            "Add one +50 card to town guard deck",
        ],
    },
    "fh-58": {
        name: "Orphan's Core",
        rewards: [
            "Gain 1 morale",
            "Gain “Key Card” (Item 246)",
            "Gain 3 collective {LUMBER}",
            "Gain 3 collective {METAL}",
            "Gain 3 collective {HIDE}",
            "Unlock {HV} class box",
            "Remove all {UNFETTERED} events from all outpost event decks",
            "Add events WO-64 and WO-65 to the winter outpost deck",
            "Add one +50 card to town guard deck",
        ],
    },
    "fh-59": {
        name: "Automation Uprising",
        rewards: [
            "Gain 2 morale",
            "Gain 2 prosperity",
            "Gain “Key Card” (Item 246)",
            "Unlock {HV} class box",
            "Remove all {UNFETTERED} events from all outpost event decks",
            "Add event SO-49 to the summer outpost deck and event WO-63 to the winter outpost deck",
            "Add one +50 card to town guard deck",
        ],
    },
    "fh-60": {
        name: "Uniting the Crown",
        rewards: [
            "Gain 2 morale",
            "Gain 1 prosperity",
            "Gain 30 experience each",
            "Unlock whichever class box is still locked: {CR} or {DT}",
            "Remove all {LURKERS} events from all outpost event decks",
            "Add event SO-48 to the summer outpost deck and event B-17 to the boat deck",
            "Add one +50 card to town guard deck",
        ],
    },
    "fh-61": {
        name: "Life and Death",
        rewards: [
            "Unlock {SH} class box",
            "Add section 140.3 to the calendar in one week",
        ],
    },
    "fh-62": {
        name: "The Unfettered Seal",
        rewards: ["Gain 1 prosperity"],
    },
    "fh-63": {
        name: "The Savvas Seal",
        rewards: ["Gain 2 prosperity"],
    },
    "fh-64": {
        name: "The Frosthaven Seal",
        rewards: ["Gain 3 morale", "Gain 3 prosperity", "Gain 3 ✔ each"],
    },
    "fh-65": {
        name: "A Strong Foundation",
        rewards: [
            "Gain 2 collective {LUMBER}",
            "Gain 2 collective {METAL}",
            "Add 95.4 to the calendar in six weeks",
        ],
    },
    "fh-66": {
        name: "Elemental Cores",
        rewards: [
            "Gain “Elemental Stone” (Item 212)",
            "Add 29.3 to the calendar in six weeks",
        ],
    },
    "fh-67": {
        name: "Core Attunement",
        rewards: ["Gain 1 morale", "Gain 1 prosperity"],
    },
    "fh-68": {
        name: "The Face of Torment",
        rewards: [
            "“Aesther Outpost” personal quest complete",
            "Add 133.1 to the calendar in two weeks",
        ],
    },
    "fh-69": {
        name: "Sacred Soil",
        rewards: [
            "Gain 1 {ARROWVINE}",
            "Gain 1 {AXENUT}",
            "Gain 1 {FLAMEFRUIT}",
            "Add 15.4 to the calendar in three weeks",
        ],
    },
    "fh-70": {
        name: "The True Oak",
        rewards: [
            "“The True Oak” personal quest complete",
            "Gain 1 Prosperity",
        ],
    },
    "fh-71": {
        name: "Invasion of the Dock",
        rewards: [
            "Damage any one even building between 71 and 89",
            "Gain 10 experience each",
            "Add 149.5 to the calendar in five weeks",
        ],
    },
    "fh-72": {
        name: "A Giant Block of Ice",
        rewards: ["Gain 1 ✔ each"],
    },
    "fh-73": {
        name: "Flotsam",
        rewards: [
            "Gain 3 collective {LUMBER}",
            "Add 146.1 to the calendar in five weeks",
        ],
    },
    "fh-74": {
        name: "Gaps in the Road",
        rewards: ["Gain one random item blueprint"],
    },
    "fh-75": {
        name: "Infiltraiting the Lair",
        rewards: ["Gain 10 gold each"],
    },
    "fh-76": {
        name: "Apotheosis",
        rewards: [
            [
                "“Threat from the Deep” personal quest complete",
                "Gain “Wave Blade” (Item 201)",
                "Add event B-18 to the boat deck",
            ],
            [],
        ],
    },
    "fh-77": {
        name: "Fish King's Ascension",
        rewards: [
            "“Threat from the Deep” personal quest complete",
            "Gain “Wave Blade” (Item 201)",
            "Add event B-19 to the boat deck",
        ],
    },
    "fh-78": {
        name: "The Lurker Problem",
        rewards: ["Gain 10 experience each"],
    },
    "fh-79": {
        name: "Relic",
        rewards: ["Gain 3 morale", "Add 45.2 to the calendar in three weeks"],
    },
    "fh-80": {
        name: "Relic Renewed",
        rewards: [
            "Gain 2 collective {LUMBER}",
            "Gain 2 collective {METAL}",
            "Gain 2 collective {HIDE}",
            "Gain 2 morale",
        ],
    },
    "fh-81": {
        name: "Ruinous Research Lab",
        rewards: [
            "Gain 1 prosperity",
            "Gain “Aesther Robe” (Item 70) blueprint",
        ],
    },
    "fh-82": {
        name: "Expedition North",
        rewards: [
            "Gain 2 prosperity",
            "Add the following Forgotten Circles items to the Frosthaven supply: 153, 159, and 161",
        ],
    },
    "fh-83": {
        name: "Rising Brine",
        rewards: ["Gain “Giant Piranha Pig Spine” (Item 202)"],
    },
    "fh-84": {
        name: "Here There Be Oozes",
        rewards: ["Gain “Ooze Vial” (Item 234)"],
    },
    "fh-85": {
        name: "Deadly Pastimes",
        rewards: ["Gain 15 experience each", "Gain 1 ✔ each"],
    },
    "fh-86": {
        name: "The Lady in White",
        rewards: ["Gain 2 morale"],
    },
    "fh-87": {
        name: "The Collection",
        rewards: [["Gain 2 inspiration"], ["Gain 1 inspiration"]],
    },
    "fh-88": {
        name: "Collection's Capstone",
        rewards: ["Gain 2 morale"],
    },
    "fh-89": {
        name: "A Contained Fire",
        rewards: ["Gain 2 collective {ROCKROOT}", "Gain 2 collective {LUMBER}"],
    },
    "fh-90": {
        name: "Frozen Treasure",
        rewards: [
            [
                "Gain 1 ✔ each",
                "Each character gains 10 gold for each numbered token they looted",
                "Lose 10 collective gold",
            ],
            [
                "Gain 1 ✔ each",
                "Each character gains 10 gold for each numbered token they looted",
                "Lose 5 collective gold",
                "All characters start the next scenario with {MUDDLE.fh} and {CURSE.fh}",
            ],
        ],
    },
    "fh-91": {
        name: "Shoreline Scramble",
        rewards: ["Gain 2 collective {LUMBER}", "Gain 1 morale"],
    },
    "fh-92": {
        name: "Sinking Ship",
        rewards: [
            "Collectively lose 5xC/2 loot tokens. Loot tokens are then traded in for loot cards",
            "Add 181.2 to the calendar in one week",
        ],
    },
    "fh-93": {
        name: "Midwinter Brawl",
        rewards: [
            ["Gain “Small Rat” (Item 235)"],
            ["Gain “Roasted Fowl” (Item 79)"],
        ],
    },
    "fh-94": {
        name: "A Grand View",
        rewards: ["Gain 10 experience each"],
    },
    "fh-95": {
        name: "To Bury the Dead",
        rewards: ["Gain 1 morale", "Gain 10 gold each"],
    },
    "fh-96": {
        name: "Underground Station",
        rewards: ["Gain 1 morale", "Gain 5 experience each"],
    },
    "fh-97": {
        name: "Program Control Nexus",
        rewards: ["Gain 5 collective {METAL}"],
    },
    "fh-98": {
        name: "Collapsing Vent",
        rewards: ["Gain 1 prosperity"],
    },
    "fh-99": {
        name: "Prison Break",
        rewards: ["Gain 10 gold each"],
    },
    "fh-100": {
        name: "Inside the Swarm",
        rewards: ["Gain 5 gold each", "Gain 10 experience each"],
    },
    "fh-101": {
        name: "Harrower Library",
        rewards: ["Gain 1 ✔ each", "Gain 10 experience each"],
    },
    "fh-102": {
        name: "Into the Black",
        rewards: ["Gain 20 experience each"],
    },
    "fh-103": {
        name: "The Lead Door",
        rewards: [
            "Gain 1 ✔ each",
            "Gain 30 experience each",
            "Gain 10 gold each",
        ],
    },
    "fh-104": {
        name: "Ruins of the Solstice",
        rewards: [
            "Gain 2 collective {SNOWTHISTLE}",
            "Add event WR-49 to the winter road deck",
        ],
    },
    "fh-105": {
        name: "Ruins of the Equinox",
        rewards: [
            "Gain 2 collective {FLAMEFRUIT}",
            "Add 159.4 to the calendar in four weeks",
        ],
    },
    "fh-106": {
        name: "The Tempus Forge",
        rewards: ["Gain “Key of Seasons” (Item 213)"],
    },
    "fh-107": {
        name: "My Private Empire",
        rewards: [
            ["Gain any 1 collective material resource"],
            ["Gain any 2 collective material resources"],
            ["Gain any 3 collective material resources"],
            ["Gain any 4 collective material resources"],
            [
                "Gain any 5 collective material resources",
                "Gain “Cup of Ventillion” (Item 214)",
            ],
        ],
    },
    "fh-108": {
        name: "Lustrous Pit",
        rewards: ["Gain 2 morale", "Gain 2 inspiration"],
    },
    "fh-109": {
        name: "Furious Factory",
        rewards: ["Gain 5 collective {METAL}"],
    },
    "fh-110": {
        name: "Guardian's Temple",
        rewards: ["Gain “Rose Guantlet” (Item 75) blueprint"],
    },
    "fh-111": {
        name: "Ice Cave",
        rewards: ["Gain “Cracked Mirror” (Item 236)"],
    },
    "fh-112": {
        name: "Raised by Wolves",
        rewards: ["Gain 15 experience each"],
    },
    "fh-113": {
        name: "Lush Grotto",
        rewards: ["Gain any 4 collective herb resources"],
    },
    "fh-114": {
        name: "Work Freeze",
        rewards: [
            "Gain 5 collective {HIDE}",
            "Add 86.1 to the calendar in four weeks",
        ],
    },
    "fh-115": {
        name: "Pylon Problems",
        rewards: [
            ["Gain 15 experience each", "Add 66.2 to the calendar in one week"],
            [
                "Gain 15 experience each",
                "Add 66.2 to the calendar in two weeks",
            ],
            [
                "Gain 15 experience each",
                "Add 66.2 to the calendar in three weeks",
            ],
            [
                "Gain 15 experience each",
                "Add 66.2 to the calendar in four weeks",
            ],
            [
                "Gain 15 experience each",
                "Add 66.2 to the calendar in five weeks",
            ],
            [
                "Gain 15 experience each",
                "Add 66.2 to the calendar in six weeks",
            ],
            [
                "Gain 15 experience each",
                "Add 66.2 to the calendar in seven weeks",
            ],
        ],
    },
    "fh-116": {
        name: "Caravan Guards",
        rewards: [
            ["Gain 1 morale"],
            [
                "Gain 1 morale",
                "Gain 1 collective {LUMBER}",
                "Gain 1 collective {METAL}",
                "Gain 1 collective {HIDE}",
            ],
            [
                "Gain 1 morale",
                "Gain 1 collective {LUMBER}",
                "Gain 1 collective {METAL}",
                "Gain 1 collective {HIDE}",
                "Gain any 2 material resources each",
                "Gain 10 experience each",
            ],
            [
                "Gain 1 morale",
                "Gain 1 collective {LUMBER}",
                "Gain 1 collective {METAL}",
                "Gain 1 collective {HIDE}",
                "Gain any 2 material resources each",
                "Gain 10 experience each",
                "Gain one random item blueprint",
            ],
            [
                "Gain 1 morale",
                "Gain 1 collective {LUMBER}",
                "Gain 1 collective {METAL}",
                "Gain 1 collective {HIDE}",
                "Gain any 2 material resources each",
                "Gain 10 experience each",
                "Gain one random item blueprint",
                "Gain 1 prosperity",
                "Gain 2 morale",
            ],
        ],
    },
    "fh-117": {
        name: "A Waiting Game",
        rewards: ["Gain 4 collective {HIDE}", "Gain 10 gold each"],
    },
    "fh-118": {
        name: "Lurker Necromancy",
        rewards: [
            "Gain “Amulet of Eternal Life” (Item 66) blueprint",
            "Trade all tokens in for loot cards",
        ],
    },
    "fh-119": {
        name: "Radiant Dust",
        rewards: [
            "Spend 7 remaining dust to gain “Scepter of Control” (Item 203)",
            "Spend 1 remaining dust to gain any 1 material resource",
            "Spend 2 remaining dust to gain any 1 herb resource",
        ],
    },
    "fh-120": {
        name: "Under the Influence",
        rewards: [
            "Gain 1 morale",
            "Gain 2 collective {LUMBER}",
            "Add 76.3 to the calendar in ten weeks",
        ],
    },
    "fh-121": {
        name: "Black Memories",
        rewards: [
            ["Add 184.3 to the calendar in six weeks"],
            ["Gain “Mind-Thieving Helmet” (Item 193)"],
            ["Gain 2 morale"],
        ],
    },
    "fh-122": {
        name: "The Eternal Crave",
        rewards: [
            ["Gain 10 gold each"],
            ["Gain 15 experience each", "Gain 1 ✔ each"],
        ],
    },
    "fh-123": {
        name: "The Titan",
        rewards: ["Gain 1 morale", "Gain 1 inspiration"],
    },
    "fh-124": {
        name: "A Growing Problem",
        rewards: ["Gain 15 experience each"],
    },
    "fh-125": {
        name: "The Longest Second",
        rewards: ["Gain “Temporal Amulet” (Item 194)"],
    },
    "fh-126": {
        name: "Joseph the Lion",
        rewards: ["Gain 10 gold each"],
    },
    "fh-127": {
        name: "Derelict Freighter",
        rewards: [
            ["Gain 10 collective gold"],
            ["Gain 10 collective gold", "Gain 2 collective {LUMBER}"],
            [
                "Gain 10 collective gold",
                "Gain 2 collective {LUMBER}",
                "Gain “Tri-Corner Hat” 195",
            ],
            [
                "Gain 10 collective gold",
                "Gain 2 collective {LUMBER}",
                "Gain “Tri-Corner Hat” 195",
                "Gain 1 prosperity",
            ],
        ],
    },
    "fh-128": {
        name: "A Tall Drunken Tale",
        rewards: [
            ["Gain 1 morale", "Add 100.3 to the calendar in three weeks"],
            ["Gain 2 morale", "Add 100.3 to the calendar in three weeks"],
            [
                "Gain 3 morale",
                "Gain “Befuddling Mug” (Item 215)",
                "Add 100.3 to the calendar in three weeks",
            ],
        ],
    },
    "fh-129": {
        name: "How to Lay an Ambush",
        rewards: ["Gain 1 ✔ each"],
    },
    "fh-130": {
        name: "And Then, a Stream",
        rewards: [
            [
                "Lose 3 morale",
                "Gain 1 prosperity",
                "Gain 10 collective gold for each token carried",
            ],
            ["Gain 5 collective gold for each token carried"],
        ],
    },
    "fh-131": {
        name: "The Dancing Iceberg",
        rewards: [
            "Gain 10 experience each",
            "Gain “Mesmerizing Seashell” (Item 237)",
        ],
    },
    "fh-132": {
        name: "Temple of Feline Power",
        rewards: ["Gain 1 inspiration"],
    },
    "fh-133": {
        name: "Bolt",
        rewards: ["Gain “Cracked Idol” (Item 238)"],
    },
    "fh-134": {
        name: "Tower of Knowledge",
        rewards: ["Gain “Tome of Conflict” (Item 219)"],
    },
    "fh-135": {
        name: "Belara's Keep",
        rewards: [
            "Gain 2 morale",
            "Gain 2 prosperity",
            "Gain 1 inspiration",
            "Gain “Belara's quill” (Item 220)",
        ],
    },
    "fh-136": {
        name: "Abandoned Hideout",
        rewards: ["Gain 20 gold each", "Gain “Ancient Coin” (Item 245)"],
    },
    "fh-137": {
        name: "Pirate Queen's Haul",
        rewards: [
            "Gain 2 prosperity",
            "Gain 3 morale",
            "Gain 30 gold each",
            "Add one +1 sticker to a money loot card",
        ],
    },
    "fh-201": {
        name: "Wonder of Nature",
        rewards: ["“Wanderer's Club” (Item 248)"],
    },
    "fh-202": {
        name: "Race Against the Clock",
        rewards: ["“Recalibration Trigger” (Item 249)"],
    },
    "fh-203": {
        name: "Scouting Ambush",
        rewards: ["“Spear of Justice” (Item 250)"],
    },
    "fh-204": {
        name: "The Dead of Night",
        rewards: ["“Soul Urn” (Item 251)"],
    },
    "fh-205": {
        name: "Bones in the Dirt",
        rewards: ["“Black Earth Codex” (Item 252)"],
    },
    "fh-206": {
        name: "Divide and Conquer",
        rewards: ["“Mask of Duality” (Item 253)"],
    },
    "fh-207": {
        name: "Path of Ancestry",
        rewards: ["“Ancestral Blade” (Item 254)"],
    },
    "fh-208": {
        name: "Crumbling Descent",
        rewards: ["“Mountain's Heart” (Item 255)"],
    },
    "fh-209": {
        name: "Tuning the Resonance",
        rewards: ["“Sonorous Shard” (Item 256)"],
    },
    "fh-210": {
        name: "A Magnificent Trap",
        rewards: ["“Sly Camouflage” (Item 257)"],
    },
    "fh-211": {
        name: "A Collection of Suffering",
        rewards: ["“Skull of Agony” (Item 258)"],
    },
    "fh-212": {
        name: "Fighting Snow with Snow",
        rewards: ["“Storm Carving” (Item 259)"],
    },
    "fh-213": {
        name: "Under the Ice",
        rewards: ["“Freezing Orb” (Item 260)"],
    },
    "fh-214": {
        name: "Recharge",
        rewards: ["“Energized Module” (Item 261)"],
    },
    "fh-215": {
        name: "Boiler Room",
        rewards: ["“Exhaust Boots” (Item 262)"],
    },
    "fh-216": {
        name: "Wet Work",
        rewards: ["“Warlord's Skull” (Item 263)"],
    },
    "fh-217": {
        name: "Crash Against the Waves",
        rewards: ["“Spiraled Medallion” (Item 264)"],
    },
    "cs-1": {
        name: "The Dark Lake",
    },
    "cs-2": {
        name: "Underground Channels",
    },
    "cs-3": {
        name: "Flooded Cavern",
        rewards: ["10 experience each", "+1 prosperity"],
    },
    "cs-4": {
        name: "Infected Warriors",
        rewards: [
            "Gain “Slug Crossbow” (Item 84)",
            "Each character gains one random “Orb” item (Item 1-10)",
            "TODO:: gain slug cross bow... what is party goal?",
        ],
    },
    "cs-5": {
        name: "Blood of the Oozes",
        rewards: [
            "Gain “Slug Crossbow” (Item 84)",
            "Each character gains one random “Orb” item (Item 1-10)",
            "TODO:: gain slug cross bow... what is party goal?",
        ],
    },
    "cs-6": {
        name: "Poisoned Water",
        rewards: [
            "+2 reputation",
            "+1 prosperity",
            "Gain “Stun Powder” (Item 21)",
        ],
    },
    "cs-7": {
        name: "Golden Eggs",
        rewards: [
            "Gain 30 collective gold",
            "Each character gains one random “Orb” item (Item 1-10)",
        ],
    },
    "cs-8": {
        name: "Shattered Fortress",
        rewards: [
            "Gain “Curious Pendant” (Item 20)",
            "Each character gains one random “Orb” item (Item 1-10)",
        ],
    },
    "cs-9": {
        name: "Rescue Mission",
        rewards: ["10 gold each", "Gain “Smog Emitter” (Item 62)"],
    },
    "cs-10": {
        name: "Den of Monstrosity",
        rewards: [
            "Gain “Minor Mana Potion” (Item 12)",
            "Gain “Minor Healing Potion” (Item 20)",
        ],
    },
    "cs-11": {
        name: "Voyage Abound",
    },
    "cs-12": {
        name: "Uncovering the Source",
    },
    "cs-13": {
        name: "Corpse Cavern",
        rewards: [
            "Gain “Bulkwark Banner” (Item 85)",
            "+1 prosperity",
            "Each character gains one of the following: “Major Stamina Potion” (Item 34), “Major Power Potion” (Item 41), “Major Mana Potion” (Item 48)\\nIf there are non of these available in the shop, gain 20 gold instead",
        ],
    },
    "cs-14": {
        name: "Cultist Cave",
        rewards: ["Gain “Remote Beetle” (Item 86)", "15 experience each"],
    },
    "cs-15": {
        name: "Ambush of Beasts",
    },
    "cs-16": {
        name: "Preto Krisanta",
        rewards: [
            "10 gold each",
            "One character gains one {+1_WHITE} enhancement to any Level 1 or X card",
        ],
    },
    "cs-17": {
        name: "Orb Extraction",
        rewards: ["+2 prosperity"],
    },
    "cs-18": {
        name: "Grab and Go",
        rewards: [
            "100 gold each This money must immediately be spent on enhancements",
        ],
    },
    "cs-19": {
        name: "Elemental Experiments",
        rewards: ["Gain “Major Mana Potion” (Item 48)"],
    },
    "cs-20": {
        name: "Midnight Ritual",
        rewards: [
            "+1 reputation",
            "+1 prosperity",
            "Each character gains one random “Orb” item (Item 1-10)",
        ],
    },
    "cs-21": {
        name: "A Fiery Death",
        rewards: ["Gain “Magma Waders” (Item 99)"],
    },
    "cs-22": {
        name: "Imp Temple",
        rewards: [
            "Each character gains +2 gold and 2 experience for each money token looted.",
        ],
    },
    "cs-23": {
        name: "Icicle Chambers",
        rewards: ["15 experience each"],
    },
    "cs-24": {
        name: "Eerie Grotto",
        rewards: ["Each character gains one random “Orb” item (Item 1-10)"],
    },
    "cs-25": {
        name: "Brightspark, Behold!",
        rewards: ["+1 reputation", "Add Road Event 57 to the deck"],
    },
    "cs-26": {
        name: "Thermal Stone Peak",
        rewards: ["20 experience each", "1 ✓ each"],
    },
    "cs-27": {
        name: "Frostbite Cavern",
        rewards: ["10 gold each", "Gain “Dizzying Tincture” (Item 58)"],
    },
    "cs-28": {
        name: "Fountain of Bones",
        rewards: ["1 ✓ each"],
    },
    "cs-29": {
        name: "Burial Chamber",
    },
    "cs-30": {
        name: "Undead Terrors",
        rewards: [["70 collective gold"], ["10 experience each"]],
    },
    "cs-31": {
        name: "Eternal Portals",
        rewards: ["1 ✓ each"],
    },
    "cs-32": {
        name: "Confronting the Past",
        rewards: [
            'Read {PAGE} 75 (<a class="desc-link" target="_blank" href="https://www.thecrimsonscales.com/endgame">Endgame</a>)',
            "Password “Water Hydra”",
        ],
    },
    "cs-33": {
        name: "Siege Tower",
        rewards: ["Gain “Blazing Boots” (Item 87)", "+1 reputation"],
    },
    "cs-34": {
        name: "Great Oak Escape",
        rewards: [
            "Unlock Envelope {BM}",
            "+2 reputation",
            "All characters add one {BLESS} to their attack modifer deck for the next scenario",
        ],
    },
    "cs-35": {
        name: "Prison Riot",
    },
    "cs-36": {
        name: "Hunters Bounty",
        rewards: ["Unlock Envelope {CG}", "10 gold each"],
    },
    "cs-37": {
        name: "Burning Stones",
        rewards: ["Gain one random “Stone” item (Item 94-100)"],
    },
    "cs-38": {
        name: "Alters of Confusion",
        rewards: ["Unlock Envelope {LU}"],
    },
    "cs-39": {
        name: "Festering Mire",
        rewards: [
            "10 experience each",
            "Remove the top 5 cards from the City Event deck and shuffle City Event 57 into them. Place the remaining City Event cards aside until City Event 57 is resolved. Any additional card that would be added to the City Event deck during this time is added after City Event 57 is resolved and the remaining cards are combined with the original deck.",
            "“An Adder Divides” quest is not yet complete",
        ],
    },
    "cs-40": {
        name: "Smugglers Hideout",
        rewards: ["Gain 50 collective gold"],
    },
    "cs-41": {
        name: "Curse of the Void",
        rewards: ["Gain 50 collective gold"],
    },
    "cs-42": {
        name: "Unnatural Beasts",
        rewards: ["+1 reputation", "10 experience each"],
    },
    "cs-43": {
        name: "Mansion Maze",
        rewards: [
            "+1 prosperity",
            "Each character gains one random “Orb” item (Item 1-10)",
        ],
    },
    "cs-44": {
        name: "Haunted Manor",
        rewards: [
            "20 gold each",
            "Each character gains one random “Orb” item (Item 1-10)",
        ],
    },
    "cs-45": {
        name: "Defend the Swamp",
        rewards: ["+1 reputation", "+1 prosperity", "1 ✓ each"],
    },
    "cs-46": {
        name: "Vermling Gardens",
        rewards: ["+2 reputation", "15 experience each"],
    },
    "cs-47": {
        name: "Viper Marsh",
        rewards: [
            "Gain “Ghost Viper Venom” (Item 55)",
            "+1 prosperity",
            "10 gold each",
        ],
    },
    "cs-48": {
        name: "Forest Fire",
        rewards: ["10 experience each", "20 gold each"],
    },
    "cs-49": {
        name: "Oak Invasion A",
        rewards: [
            "During the next visit to Gloomhaven, donations to the Sanctuary  cost 0 gold",
        ],
    },
    "cs-50": {
        name: "Oak Invasion B",
        rewards: ["10 experience each", "1 ✓ each"],
    },
    "cs-51": {
        name: "Rodent Warehouse",
        rewards: ["+1 reputation", "30 gold each"],
    },
    "cs-52": {
        name: "Wishing Well",
        rewards: ["Gain “Drakescale Shield” (Item 37)"],
    },
    "cs-53": {
        name: "Cave of Currents",
        rewards: ["Gain “Sphere of Currents” (Item 83)", "5 gold each"],
    },
    "cs-54": {
        name: "Lair of Horrors",
        rewards: ["+1 reputation", "30 gold each"],
    },
    "cs-55": {
        name: "Catacomb Plunder",
        rewards: ["+1 prosperity", "+1 reputation", "10 experience each"],
    },
    "cs-56": {
        name: "Disarm the Machine",
        rewards: ["Gain “Chain Cannon” (Item 45)"],
    },
    "cs-57": {
        name: "Elemental Converter",
        rewards: ["Gain “Test Tube” (Item 48)"],
    },
    "cs-58": {
        name: "Penitentiary Lockdown",
        rewards: [
            "Gain one of the following as an item and the other as an item design:",
            "“Claw Trap” (Item 34)",
            "“Clamp Trap” (Item 56)",
        ],
    },
    "cs-59": {
        name: "Forest Invaders",
        rewards: ["Gain “Nature Armlets” (Item 47)"],
    },
    "cs-60": {
        name: "Through the Fire and Flames",
        rewards: ["Gain “Flaming Armor” (Item 51)"],
    },
    "cs-61": {
        name: "Archaic Preservation",
        rewards: ["Gain “Robe of Soothing” (Item 42)"],
    },
    "cs-62": {
        name: "Hallow Redemption",
        rewards: ["Gain “Cleansed Voidheart” (Item 57)"],
    },
    "cs-63": {
        name: "A Cornered Viper",
        rewards: ["Gain “Field Mortar” (Item 53)"],
    },
    "cs-64": {
        name: "Glowing Crystals",
        rewards: ["Gain “Luminous Carapace” (Item 50)"],
    },
    "cs-65": {
        name: "Glowing Crystals",
        rewards: ["Gain “Spirit Libation” (Item 41)"],
    },
    "cs-66": {
        name: "Otherworldly Strangers",
        rewards: ["Gain “Moonleather Boots” (Item 40)"],
    },
    "cs-AA1": {
        name: "The Riches of the Steelsilk",
        rewards: ["“Rockspine Bulwark” (Item AA01)"],
    },
    "cs-AA2": {
        name: "Malign Echoes",
        rewards: ["“Ghostshimmer Honey” (Item AA02)"],
    },
    "cs-AA3": {
        name: "Hive Guardian",
        rewards: ["“Steelsilk Spinneret” (Item AA03)"],
    },
    "cs-QA1": {
        name: "Capstone Test",
        rewards: [
            [
                "“Ingenious Inventor” quest complete",
                "Open envelope {QA}",
                "“Steam Piston Hammer” design (Item QA02)",
                "“Spider Climb Gloves” design (Item QA03)",
            ],
            [
                "“Ingenious Inventor” quest complete",
                "Open envelope {QA}",
                "Gain one of the following: “Steam Piston Hammer” (Item QA02) OR “Spider Climb Gloves” (Item QA03)",
                "Add a copy of the chosen item to the city's available supply",
                "Add the other item to the random item design deck",
            ],
            [
                "“Ingenious Inventor” quest complete",
                "Open envelope {QA}",
                "“Steam Piston Hammer” (Item QA02)",
                "“Spider Climb Gloves” (Item QA03)",
                "Add copies to the city's available supply",
            ],
        ],
    },
    "cs-QA2": {
        name: "Mother Lode",
        rewards: [
            "“Scrap Diver” quest complete",
            "Open envelope {QA}",
            "15xA gold each (where A is the number of Ancient Scrap recovered",
            "Add City Event QA-03 to the deck",
        ],
    },
    "cs-QA3": {
        name: "Magnum Opus",
        rewards: ["“Auscudee of the Makers” (Item QA06)"],
    },
    "cs-RM1": {
        name: "Mind your Manners",
    },
    "cs-RM2": {
        name: "Sinking Kingpin",
        rewards: [
            "“Brutal Enforcer” quest complete",
            "Open Envelope {RM}",
            "+1 prosperity",
        ],
    },
    "cs-RM3": {
        name: "Ruined Colony",
    },
    "cs-RM4": {
        name: "Terrorscale Lair",
        rewards: [
            "“Apex Predator” quest complete",
            "Open Envelope {RM}",
            "“Terrorscale Saber” design (Item RM03)",
            "“Terrorscale Crossblades” design (Item RM04)",
        ],
    },
    "cs-RM5": {
        name: "Unseen Plight",
        rewards: [
            [
                "40 collective gold",
                "“Studded Leather” design (Item 030) OR “Boots of Dashing” design (Item 036)",
            ],
            ["2 ✓ each", "“Lucky Eye” design (Item 063)"],
        ],
    },
    "cs-RM6": {
        name: "The Fall of Shadow's Respite",
        rewards: [
            "“Rite of Ruin” design (Item RM05)",
            "If at least three Vermlings were alive at the end of the eighth round: 5 gold",
            "If at least five Vermlings were alive at the end of the eighth round: ✓",
        ],
    },
    "cs-RM7": {
        name: "Insatiable",
        rewards: ["10 experience each", "“Savage Cuirass” design (Item RM06)"],
    },
};
